...
public interface SpProductDetailRepository extends ProductDetailRepository {
    ...
    @Query(value = """
            select PD.id, PD.code, P.name as product, C.code as color, B.name as brand, S.name as sole,
             ...
             join sole S on PD.id_sole = S.id
             join material M on PD.id_material = M.id
             join category CA on PD.id_category = PD.id
             join size SI on PD.id_size = SI.id
             join image I on PD.id_image = I.id
    ...
    @Query(value = """
            select PD.id, PD.code, P.name as product, C.code as color, B.name as brand, S.name as sole,
             ...
             join category CA on PD.id_category = PD.id
             join size SI on PD.id_size = SI.id
             join image I on PD.id_image = I.id""", nativeQuery = true)
    Optional<ProductDetailResponse> getById(@Param("id") int id);
}
